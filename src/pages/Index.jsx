        ...
        <List width="100%">
          {tasks.map(task => (
            <ListItem key={task.id} display="flex" justifyContent="space-between" alignItems="center" p={2} bg={task.isCompleted ? 'green.100' : 'gray.100'}>
              ...
              <Flex>
                <IconButton aria-label="Complete task" icon={<FaCheck />} onClick={() => toggleTaskCompletion(task.id)} colorScheme="green" size="sm" mr={2} />
                <IconButton aria-label="Delete task" icon={<FaTrash />} onClick={() => removeTask(task.id)} colorScheme="red" size="sm" />
              </Flex>
            </ListItem>
          ))}
        </List>
